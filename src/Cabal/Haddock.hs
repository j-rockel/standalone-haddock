        ...
        doExe com = case (compToExe com) of
          Just exe -> do
            withTempDirectoryEx verbosity tmpFileOpts (buildDir lbi') "tmp" $
              \tmp -> do
                exeArgs <- fromExecutable verbosity tmp lbi' clbi computePath
                             version exe
      ...
      case component of
        CLib lib -> do
          withTempDirectoryEx verbosity tmpFileOpts (buildDir lbi) "tmp" $
            \tmp -> do
              smsg
              libArgs <- fromLibrary verbosity tmp lbi' clbi computePath
                           version lib
        ...
        CFLib flib -> (when (flag haddockForeignLibs) $ do
          withTempDirectoryEx verbosity tmpFileOpts (buildDir lbi') "tmp" $
            \tmp -> do
              smsg
              flibArgs <- fromForeignLib verbosity tmp lbi' clbi computePath
                            version flib
...
mkHaddockArgs :: Verbosity
              ...
              -> BuildInfo
              -> IO HaddockArgs
mkHaddockArgs verbosity tmp lbi clbi computePath haddockVersion inFiles bi = do
...
fromLibrary :: Verbosity
            ...
            -> Library
            -> IO HaddockArgs
fromLibrary verbosity tmp lbi clbi computePath haddockVersion lib = do
    inFiles <- map snd `fmap` getLibSourceFiles verbosity lbi lib clbi
    args    <- mkHaddockArgs verbosity tmp lbi clbi computePath haddockVersion
...
fromExecutable :: Verbosity
               ...
               -> Executable
               -> IO HaddockArgs
fromExecutable verbosity tmp lbi clbi computePath haddockVersion exe = do
    inFiles <- map snd `fmap` getExeSourceFiles verbosity lbi exe clbi
    args    <- mkHaddockArgs verbosity tmp lbi clbi computePath
...
fromForeignLib :: Verbosity
               ...
               -> ForeignLib
               -> IO HaddockArgs
fromForeignLib verbosity tmp lbi clbi computePath haddockVersion flib = do
...
haddockPackagePaths ipkgs computePath = do
...
haddockPackageFlags verbosity lbi clbi computePath = do
  ...
  haddockPackagePaths (PackageIndex.allPackages transitiveDeps) computePath
